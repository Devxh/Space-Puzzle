...
func _process(delta: float):
	...
	if newPosition.y > initialPosition.y + 308:  # Change 239 to the desired movement range
		newPosition.y = initialPosition.y + 308
		direction = -1  # Reverse direction to move downwards
	elif newPosition.y < initialPosition.y - 308:  # Change 239 to the desired movement range
		newPosition.y = initialPosition.y - 308
